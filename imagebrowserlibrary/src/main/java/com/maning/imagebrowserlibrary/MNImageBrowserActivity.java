...
public class MNImageBrowserActivity extends AppCompatActivity
{
	...
	private void initData(){
		...
		if(imageUrlList.size()<=1){
			...
			if(indicatorType==ImageBrowserConfig.IndicatorType.Indicator_Number){
				...
				numberIndicator.setText(
						String.valueOf((currentPosition+1)+"/"+imageUrlList.size()));
			}else{
	...
	private class MyAdapter extends PagerAdapter
	{
		...
		public Object instantiateItem(ViewGroup container, final int position){
			View inflate=
					layoutInflater.inflate(R.layout.mn_image_browser_item_show_image, container,
							false);
			final PhotoView imageView=(PhotoView)inflate.findViewById(R.id.imageView);
			final RelativeLayout rl_browser_root=
					(RelativeLayout)inflate.findViewById(R.id.rl_browser_root);
			final RelativeLayout progress_view=
					(RelativeLayout)inflate.findViewById(R.id.progress_view);
			...
			imageView.setOnClickListener(new View.OnClickListener()
			{
				...
				public void onClick(View view){
					...
					if(onClickListener!=null){
						onClickListener.onClick(MNImageBrowserActivity.this, imageView, position,
								url);
					}
					finishBrowser();
				}
			...
			imageView.setOnLongClickListener(new View.OnLongClickListener()
			{
				...
				public boolean onLongClick(View view){
					if(onLongClickListener!=null){
						onLongClickListener.onLongClick(MNImageBrowserActivity.this, imageView,
								position, url);
					}
					return false;
				}
		...
		if(sActivityRef!=null && sActivityRef.get()!=null){
			if(sActivityRef.get().imageUrlList.size()>1){
				...
				if(sActivityRef.get().currentPosition>=sActivityRef.get().imageUrlList.size() &&
				   sActivityRef.get().currentPosition>=1){
